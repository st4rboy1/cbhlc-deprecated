      ...
      <div className="wrapper fixed top-1/2 left-1/2 w-[400px] p-10 bg-white/10 border-2 border-black rounded-2xl backdrop-blur-md shadow-[0_0_30px_rgba(0,0,0,0.5)] text-center text-white overflow-hidden transition-transform duration-300 z-[200] h-[600px]" style={{ transform: 'translate(-50%, -50%) scale(0)' }}>
        ...
        <div className="form-box register absolute top-0 left-0 w-full mt-5 text-black" style={{ transform: 'translateX(400px)' }}>
          ...
          <form id="registerForm">
            <br />
            <div className="input-box mb-4">
              <input type="text" name="name" placeholder="Name" required className="w-[90%] py-3 px-4 rounded-md border-none outline-none text-base border-2 border-black mt-2.5" />
            </div>
            <div className="input-box mb-4">
              <input type="email" name="email" placeholder="Email" required className="w-[90%] py-3 px-4 rounded-md border-none outline-none text-base border-2 border-black mt-2.5" />
            </div>
            <div className="input-box mb-4">
              <input type="password" name="password" placeholder="Password" required className="w-[90%] py-3 px-4 rounded-md border-none outline-none text-base border-2 border-black mt-2.5" />
            </div>
            <div className="input-box mb-4">
              <select name="role" required className="w-[90%] py-3 px-4 rounded-md border-none outline-none text-base border-2 border-black mt-2.5 bg-white cursor-pointer">
                <option value="" disabled selected>Select Role</option>
                <option value="parent">Parent</option>
                <option value="registrar">Registrar</option>
              </select>
            </div>
            <button type="submit" className="btn w-[80%] py-3 border-2 border-black rounded-md bg-transparent text-black text-lg font-semibold cursor-pointer transition-all duration-300 mt-5 hover:bg-[#2c3e50] hover:text-white" id="registerBtn">Register</button>
            <div className="register-link mt-4 text-sm">
              <p>Already have an account? <a href="#" className="login-link-btn text-black underline font-semibold cursor-pointer transition-colors duration-300 ease-in-out hover:text-gray-500">Login</a></p>
            </div>
          </form>
